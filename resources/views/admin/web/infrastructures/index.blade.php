        ...
        <div class="row">
            <div class="col-sm-12">
                <div class="card">
                    ...
                    <div class="card-block">
                        <form id="buildingInfrastructuresForm" class="needs-validation" method="POST"
                            action="{{ route($route . '.store', ['departmentId' => $departmentId, 'section' => $section]) }}"
                            enctype="multipart/form-data">
                            @csrf
                            <div class="row">
                                ...
                                $buildings = json_decode('[{"title": "Main Engineering Building", "imageFile":
                                "main_building.jpg", "description": "This is the main building housing engineering
                                classes and laboratories."}, {"title": "Research Facility", "imageFile":
                                "research_facility.jpg", "description": "Dedicated research facility for engineering
                                projects."}]', true);
                                ...
                                <div class="building-entry row mb-3">
                                    <div class="form-group col-md-4">
                                        <input type="text" class="form-control" name="buildings[{{ $index }}][title]"
                                            placeholder="Building Title" value="{{ $building['title'] }}" required>
                                    </div>
                                    <div class="form-group col-md-4">
                                        <input type="file" class="form-control"
                                            name="buildings[{{ $index }}][imageFile]" accept="image/*"
                                            placeholder="Image">

                                        @if(isset($building['imageFile']))
                                        <img src="{{ asset('uploads/infrastructures/' . $building['imageFile']) }}"
                                            alt="{{ $building['title'] }}" width="70" height="70">
                                        @endif
                                    </div>
                                    <div class="form-group col-md-4">
                                        <textarea class="form-control" name="buildings[{{ $index }}][description]"
                                            placeholder="Building Description"
                                            required>{{ $building['description'] }}</textarea>
                                    </div>
                                    <div class="form-group col-md-12 text-end">
                                        <button type="button" class="btn btn-danger"
                                            onclick="removeBuildingEntry(this)">Remove</button>
                                    </div>
                                </div>
                                @endforeach
                            </div>
                            ...
                            <!-- Add Building Button -->
                            <div class="text-center">
                                <button type="button" id="addBuildingBtn" class="btn btn-info"
                                    onclick="addBuilding()"><i class="fa fa-plus"></i> Add Building</button>
                            </div>
